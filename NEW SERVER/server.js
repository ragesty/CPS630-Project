    ...
    function updateUsernameString(room) {
        ...
        if (isStarted) {
            ...
            if (team1Arr === undefined || team1Arr.length === 0) {
                // if (socket.team === 2)
                socket.emit('endGameWin', io.sockets.adapter.rooms[currentRoom].points2, socket.points, io.sockets.adapter.rooms[currentRoom].points1);
            } else if (team2Arr === undefined || team2Arr.length === 0) {
                // if (socket.team === 1)
                socket.emit('endGameWin', io.sockets.adapter.rooms[currentRoom].points1, socket.points, io.sockets.adapter.rooms[currentRoom].points2);
            } else
                console.log('Teams are not empty');
